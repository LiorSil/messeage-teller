...
const getContact = async (req: Request, res: Response) => {
  try {
    ...
    const subContacts = await Promise.all(
      contact.subContacts.map(async (subContactId) => {
        const fullSubContact = await contactService.getContactById(
          subContactId
        );
        if (!fullSubContact) return null;
        const { _id, name, phoneNumber, avatar } = fullSubContact;
        return { _id, name, phoneNumber, avatar };
      })
    );

    const { _id, name, avatar, createdAt } = contact;
    res.status(200).json({
      _id,
      name,
      avatar,
      phoneNumber,
      createdAt,
      subContacts: subContacts.filter(Boolean), // Filter out any null values
    });
  } catch (error) {
    handleError(res, error);
  }
